...
import { Clock, ChevronLeft, ChevronRight, Send, AlertCircle, ShieldCheck } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F5F5F0] flex flex-col">
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serif font-bold text-lg leading-none">{tryout?.title}</h1>
            <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1">Question {currentIdx + 1} of {questions.length}</p>
          </div>
        </div>

        <div className="flex items-center gap-6">
          <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-mono font-bold ${timeLeft < 300 ? 'bg-red-50 text-red-500' : 'bg-[#F5F5F0] text-[#5A5A40]'}`}>
            <Clock size={18} />
            {formatTime(timeLeft)}
          </div>
          <button 
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-7xl mx-auto w-full p-6 gap-8">
            ...
            >
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${
                  currentQuestion?.difficulty === 'easy' ? 'bg-green-50 text-green-500' :
                  currentQuestion?.difficulty === 'medium' ? 'bg-yellow-50 text-yellow-500' : 'bg-red-50 text-red-500'
                }`}>
              ...
              <div className="space-y-4">
                {currentQuestion?.options.map((option: string, idx: number) => (
                  ...
                  >
                    ...
                    <span className={`font-medium ${answers[currentQuestion.id] === idx ? 'text-[#1a1a1a]' : 'text-gray-600'}`}>
                      {option}
                    </span>
                  </button>
                ))}
        ...
        <div className="w-full lg:w-80 space-y-6">
          <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100">
            <h3 className="font-bold text-sm mb-4 uppercase tracking-widest text-gray-400">Question Navigator</h3>
          ...
          <div className="bg-orange-50 rounded-[32px] p-6 border border-orange-100">
            ...
            <p className="text-[10px] text-orange-600/70 font-serif italic">
